<div class="absolute top-0 w-full overflow-hidden border-b z-50 backdrop-blur-sm h-8 flex items-center transition-transform duration-500"
    :class="{ '-translate-y-full': scrolled @if(request()->path() === '/' || request()->path() === 'campaign'), 'border-white/5': true @else, 'border-amber-900/30 bg-orange-50': true @endif }">
                    ...
                    <button @mouseenter="openDropdown = true" @mouseleave="openDropdown = false"
                        class="text-sm font-semibold transition-colors flex items-center gap-1 uppercase tracking-wide py-2
                               @if(request()->is('about/*'))
                                ...
                                @endif">
                        About <i class="fas fa-chevron-down text-[9px] transition-transform duration-300" :class="{ 'rotate-180': openDropdown @if(request()->path() === '/' || request()->path() === 'campaign'), 'text-saffron-500': true @else, 'text-saffron-600': true @endif }"></i>
                    </button>
                    <div x-show="openDropdown" @mouseenter="openDropdown = true" @mouseleave="openDropdown = false" x-transition.opacity
                        ...
                        <a href="/about/bhaktivinoda-thakura"
                            class="block px-6 py-3 text-xs transition
                               ...
                               @endif">
                            Who is Srila Bhaktivinoda Thakur</a>
                        ...
                        <a href="/about/message-from-project-director"
                            class="block px-6 py-3 text-xs transition
                               @if(request()->is('about/message-from-project-director'))
                                   font-bold bg-saffron-500/20 text-saffron-700 hover:bg-saffron-500/30 hover:text-saffron-800
                               @elseif(request()->path() === '/' || request()->path() === 'campaign')
                                   text-stone-400 hover:bg-saffron-500/10 hover:text-saffron-300
                               @else
                ...
                <div x-show="open" @click.away="open = false"
                    ...
                    <div x-data="{ aboutOpen: false }" class="@if(request()->path() === '/' || request()->path() === 'campaign') text-stone-300 @else text-amber-950 @endif">
                        ...
                        <div x-show="aboutOpen" x-transition class="pl-4 space-y-1 mt-1">
                            ...
                            <a href="/about/message-from-project-director" class="block py-2.5 px-4 rounded-lg text-sm transition-colors
                                    @if(request()->path() === '/' || request()->path() === 'campaign')
                                        text-stone-400 hover:bg-saffron-500/10 hover:text-saffron-300
                                    @else
                                        text-amber-900 hover:bg-orange-100
                                    @endif">Project Director's Message</a>
                            ...
                            <a href="/about/gbc" class="block py-2.5 px-4 rounded-lg text-sm transition-colors
                                    @if(request()->path() === '/' || request()->path() === 'campaign')
                                        text-stone-400 hover:bg-saffron-500/10 hover:text-saffron-300
                                    @else
                                        text-amber-900 hover:bg-orange-100
                                    @endif">GBC Resolution</a>
                        </div>
                    </div>
